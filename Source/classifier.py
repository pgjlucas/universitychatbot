...
def classifier(question):
	...
	neigh = KNeighborsClassifier(n_neighbors=5, weights='distance', algorithm='auto',p=2,metric='minkowski')
	RF = RandomForestClassifier(n_estimators=15, max_depth=30, random_state=0, bootstrap=True)
	Bagging_DTree = BaggingClassifier(base_estimator=DTree, n_estimators=20, max_samples=0.8, max_features=1.0)
	Bagging_neigh = BaggingClassifier(base_estimator=neigh, n_estimators=20, max_samples=0.8, max_features=1.0)
	...
	DTree.fit(X, Y)
	RF.fit(X, Y)
	Bagging_DTree.fit(X, Y)
	Bagging_neigh.fit(X, Y)	
	#clf = RandomForestClassifier(n_estimators=10, max_depth=15, random_state=0, bootstrap=True)
	#clf.fit(X, Y)
	...
	label1= neigh.predict(numeric_features_vector)
	label2= DTree.predict(numeric_features_vector)
	label3= RF.predict(numeric_features_vector)
	label4= Bagging_DTree.predict(numeric_features_vector)
	label5= Bagging_neigh.predict(numeric_features_vector)
	label_list = [label1[0],label2[0],label3[0],label4[0],label5[0]] 
	label = Counter(label_list).most_common(1)
	return label[0][0]
